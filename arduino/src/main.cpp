...
void LCD_showInfo(void *params)
{
  for (;;)
  {
    sprintf(message, "Temp:%d Light:%d", temperature, brightness);
    lcd.clear();
    lcd.println(message);
...
void read_temperature_task(void *params)
{
  ...
  {
    Serial.println("T");
    temperature = analogRead(LM35_PORT) * 500.0 / 1024;
    Serial.println(temperature);
    write_on_sdCard('T', temperature);
    vTaskDelay(TOTAL_DELAY / portTICK_PERIOD_MS);
  }
  vTaskDelete(NULL);
}
...
void read_brightness_task(void *params)
{
  for (;;)
  {
    ...
    brightness = map(value, 0, 1023, 0, 100);
    Serial.println(brightness);
    write_on_sdCard('L', brightness);
    vTaskDelay(TOTAL_DELAY / portTICK_PERIOD_MS);
  }
  vTaskDelete(NULL);
}
